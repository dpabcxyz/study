<template>
  <div class="systemTool" >
      <ul class="toolItem" :style="{color:navTopTxt}">
          <li class="symbol"><svg class="icon" aria-hidden="true"><use xlink:href="#search"></use></svg></li>
      </ul>
     <el-dropdown trigger="click" class="toolItem" :style="{color:navTopTxt}" @command="handleCommand">
       ...
       <el-dropdown-menu slot="dropdown">
           <el-dropdown-item class="clearfix" command="zh">
           中文
         </el-dropdown-item>
     ...
     <el-dropdown trigger="click" class="toolItem" :style="{color:navTopTxt}">
    ...
    <el-dropdown class="toolItem" :style="{color:navTopTxt}" trigger="click" @command="themeChange">
      <span class="el-dropdown-link" >
...
<script>
  ...
  export default{
    ...
    data:function(){
      return {
          themeData:[
              {id:'01',val:'blue',txt:'深蓝风'},
              {id:'02',val:'green',txt:'墨绿风'},
              {id:'03',val:'dark',txt:'厚重风'},
              {id:'04',val:'light',txt:'简洁风'},
          ]
      }
    },
    computed: {
        ...mapGetters(['navTopTxt']),
    },
    created(){
    },
    methods: {
      ...
      themeChange(val){
        this.$store.commit(types.CURRTHEME,val);
      }
    },
  }
</script>
